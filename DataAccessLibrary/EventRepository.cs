...
public class EventRepository : IEventRepository
{
        ...
        using (var connection = new MySqlConnection(ConnectionString.GetString()))
        {
            ...
            using (var transaction = connection.BeginTransaction())
            {
                try
                {
                    ...
                    var eventTemp = CreateEvent(startTime, endDate, descriptions, name, maxParticipants, boats,
        ...
        using (var connection = new MySqlConnection(ConnectionString.GetString()))
        {
            ...
            using (var transaction = connection.BeginTransaction())
            {
                try
                {
                    ...
                    var eventTemp = ChangeEvent(events, startDate, endDate, description, name, maxParticipants,
                        connection, transaction);
        ...
        int maxParticipants, List<Boat> boats, MySqlConnection connection, MySqlTransaction transaction)
    {
        ...
        using (var command = new MySqlCommand(sql, connection))
        {
            ...
            command.Transaction = transaction;
            command.ExecuteNonQuery();
            return new Event(new List<Member>(), startTime, endDate, descriptions, name, (int)command.LastInsertedId,
                maxParticipants, boats);
        }
    }

    private void UpdateReservation(int reservation, DateTime startDate, DateTime endDate, MySqlConnection connection,
        MySqlTransaction transaction)
    {
        const string sql =
    ...
    private List<int> GetEventReservationsIds(Event events)
    {
        ...
        using (var connection = new MySqlConnection(ConnectionString.GetString()))
        {
            ...
            using (var command = new MySqlCommand(sql, connection))
            {
                ...
                {
                    while (reader.Read()) list.Add(reader.GetInt32("reservation_id"));
                }
            }
        }
