...
public class EventService
{
    ...
    public Event CreateEvent(DateTime startDate, DateTime endDate, string description, string name, int maxParticipants,
        List<Boat> boats, Member loggedInMember)
    {
        ...
        if (startDate < DateTime.Now.AddDays(14)) throw new EventTimeException();
        if (startDate > endDate) throw new InvalidTimeException();
    ...
    public Event UpdateEvent(Event events, DateTime startDate, DateTime endDate, string description, string name,
        int maxParticipants, Member loggedInMember)
    {
        if (!(loggedInMember.Roles.Contains("evenementen_connissaris") || loggedInMember.Roles.Contains("beheerder")))
            throw new IncorrectRightsException();
        if (startDate < DateTime.Now.AddDays(14)) throw new EventTimeException();
