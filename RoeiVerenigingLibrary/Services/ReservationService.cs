...
public class ReservationService(IReservationRepository reservationRepository)
{
    ...
    public Reservation Create(Member member, int boatId, DateTime startTime, DateTime endTime)
    {
        ...
        if (!member.Roles.Contains("beheerder"))
        {
            ...
            {
                var message = "Je kan voor maximaal " + _maxReservationTime.Hours + " uur reserveren!";
            ...
            if (MaxReservations(member)) throw new MaxAmountOfReservationExceeded();
    ...
    {
        var maxReservationTime = new TimeSpan(2, 0, 0);
    ...
    {
        var solarTimes = new SolarTimes(selectedDate.Date, 52.5125, 6.09444);
        var cet = TimeZoneInfo.FindSystemTimeZoneById("W. Europe Standard Time");
        var sunrise = TimeZoneInfo.ConvertTimeFromUtc(solarTimes.Sunrise.ToUniversalTime(), cet);

        var roundedSunrise = sunrise.AddMinutes(-sunrise.Minute).AddSeconds(-sunrise.Second)
            .AddMilliseconds(-sunrise.Millisecond)
            .AddMicroseconds(-sunrise.Microsecond);
        if (roundedSunrise != sunrise) roundedSunrise = roundedSunrise.AddHours(1);
    ...
    public List<DateTime> GetAvailableTimes(DateTime selectedDate, List<Boat> boatList)
    {
        ...
        var startTime = selectedDate.AddHours(Sunrise(selectedDate).Hour);
        var endTime = selectedDate.AddHours(Sunset(selectedDate).Hour);
        ...
        var reservationsPerTimeBlock = reservations
        ...
        var timeAvailableList = Enumerable.Range(0, (endTime - startTime).Hours)
