...
{
    private readonly BoatService _boatService = new(new BoatRepository());
    private readonly Member _loggedInMember;

    private readonly ReservationService _reservationService = new(new ReservationRepository());
    private readonly List<Boat> _boatList;
    private readonly List<Reservation> _reservationsList;
    private Boat _selectedBoat;
    private readonly List<Button> _selectedButtons = new();
    private DateTime _selectedDate;
    private readonly List<DateTime> _selectedTimes = new();
            ...
            if (_selectedTimes.Count < 2)
            {
                ...
                if (_selectedTimes.Count == 2)
                {
                    ...
                    if (difference != TimeSpan.FromHours(1))
                    {
                        ...
                        foreach (var button in _selectedButtons) button.Background = CustomColors.MainBackgroundColor;
    ...
    {
        var availableBoatList = _boatList.Where(boat => !_reservationsList
        ...
        foreach (var boat in availableBoatList)
        {
            ...
            button.Click += (sender, e) =>
            {
                ...
                var boatId = (int)((Button)sender).Tag;

                var selectedBoat = _boatList.FirstOrDefault(b => b.Id == boatId);
                if (selectedBoat != null) _selectedBoat = selectedBoat;
            };
